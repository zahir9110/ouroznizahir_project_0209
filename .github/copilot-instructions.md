...
- Key libs in use: **flutter_bloc**, **get_it**, **dio**, **flutter_screenutil**, **flutter_secure_storage**, **shared_preferences**. See `pubspec.yaml`.
...
- Network and DI are prepped but not fully wired:
  ...
  - `get_it` is included for DI but no global locator exists yet — add `lib/core/di/locator.dart` and register services there.
...
```
Call `setup()` early (e.g., in `main()` before `runApp`).
